...
export function PLYPointCloud({ 
  ...
  const [plyData, setPlyData] = useState<THREE.BufferGeometry | null>(null)
  ...
  const { positions, colors, normals, sampledCount } = useMemo(() => {
    ...
    const positionAttribute = plyData.attributes.position
    const colorAttribute = plyData.attributes.color
    const normalAttribute = plyData.attributes.normal
  ...
  const hasVertexColors = !!plyData?.attributes.color
